    ...
    <div class="grid-top">
      ...
      <div>
        ...
        <div class="profile">
          ...
          <div class="name">
            ...
            @if($lastName)<br>{{ strtoupper($lastName) }}@endif
          </div>
        </div>

        <div class="info-list">
          <div><span class="label">📅 Tanggal Lahir</span><span class="val">: {{ $assessment->dob ? \Carbon\Carbon::parse($assessment->dob)->translatedFormat('d F Y') : '-' }}</span></div>
          <div><span class="label">👤 Jenis Kelamin</span><span class="val">: {{ ucfirst($assessment->gender ?? '-') }}</span></div>
          <div><span class="label">📞 Nom. Telepon</span><span class="val">: {{ $assessment->phone ?? '-' }}</span></div>
          <div><span class="label">✉️ Email</span><span class="val">: {{ $assessment->email ?? '-' }}</span></div>
          @if($assessment->group_id)
          <div><span class="label">💼 Jabatan</span><span class="val">: {{ $assessment->position ?? '-' }}</span></div>
          <div><span class="label">🏢 Grup/Tim</span><span class="val">: {{ $assessment->group ? $assessment->group->name : '-' }}</span></div>
          @else
          <div><span class="label">💼 Pekerjaan</span><span class="val">: {{ $assessment->job ?? '-' }}</span></div>
          @endif
          <div><span class="label">📅 Tanggal Tes</span><span class="val">: {{ $assessment->created_at ? $assessment->created_at->translatedFormat('d F Y') : date('d M Y') }}</span></div>
      ...
      <div class="right-col">
        ...
        <div class="row-2">
              ...
              <h2>{!! nl2br(e(strtoupper($arch['name'] ?? $assessment->archetype_name))) !!}</h2>
              <div class="archetype-icon">dY -</div>
              <p>{!! nl2br(e($ai_narasi['archetype_box_desc'] ?? ($arch['description'] ?? ''))) !!}</p>
              ...
              <div style="margin-top: 24px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 16px;">
                <div style="margin-bottom: 12px;">
                  ...
                  <span style="font-size: 12px; color: #fff;">{{ $arch['core_desire'] ?? '' }}</span>
                </div>
                <div style="margin-bottom: 24px;">
                  ...
                  <span style="font-size: 12px; color: #fff;">{{ $arch['core_fear'] ?? '' }}</span>
                </div>
                ...
                <div style="display: flex; flex-direction: column; gap: 15px;">
                    <div>
                      ...
                      <ul style="padding-left: 16px; margin: 0; font-size: 11.5px; color: #c7cde0; line-height: 1.5;">
                        @foreach($ai_narasi['strengths_in_action'] ?? ($arch['strengths'] ?? []) as $str)
                          <li style="margin-bottom: 8px;"><b style="color:#fff;">{{ $str['title'] ?? '' }}</b>: {{ $str['desc'] ?? '' }}</li>
                        @endforeach
                      </ul>
                    </div>
                    <div>
                      ...
                      <ul style="padding-left: 16px; margin: 0; font-size: 11.5px; color: #c7cde0; line-height: 1.5;">
                        @foreach($ai_narasi['growth_opportunities'] ?? ($arch['blindspots'] ?? []) as $bs)
                          <li style="margin-bottom: 8px;"><b style="color:#fff;">{{ $bs['title'] ?? '' }}</b>: {{ $bs['desc'] ?? '' }}</li>
                        @endforeach
                      </ul>
                    </div>
                  </div>
        ...
        <div class="section-body">
          ...
          <div class="driver-row">
            ...
            <div>
              ...
              <p class="driver-desc">{{ $driverLevels['security']['level_info']['desc'] ?? '' }}</p>
              <div class="tags"><span class="label">Kekuatan Anda</span>
                @foreach(array_slice($driverLevels['security']['level_info']['characteristics'] ?? [], 0, 3) as $tag)
                  <span>{{ $tag }}</span>
                @endforeach
              </div>
            </div>
          ...
          <div class="driver-row">
            ...
            <div>
              ...
              <p class="driver-desc">{{ $driverLevels['significance']['level_info']['desc'] ?? '' }}</p>
              <div class="tags"><span class="label">Kekuatan Anda</span>
                @foreach(array_slice($driverLevels['significance']['level_info']['characteristics'] ?? [], 0, 3) as $tag)
                  <span>{{ $tag }}</span>
                @endforeach
              </div>
            </div>
          ...
          <div class="driver-row">
            ...
            <div>
              ...
              <p class="driver-desc">{{ $driverLevels['connection']['level_info']['desc'] ?? '' }}</p>
              <div class="tags"><span class="label">Kekuatan Anda</span>
                @foreach(array_slice($driverLevels['connection']['level_info']['characteristics'] ?? [], 0, 3) as $tag)
                  <span>{{ $tag }}</span>
                @endforeach
              </div>
            </div>
          ...
          <div class="driver-row">
            ...
            <div>
              ...
              <p class="driver-desc">{{ $driverLevels['growth']['level_info']['desc'] ?? '' }}</p>
              <div class="tags"><span class="label">Kekuatan Anda</span>
                @foreach(array_slice($driverLevels['growth']['level_info']['characteristics'] ?? [], 0, 3) as $tag)
                  <span>{{ $tag }}</span>
                @endforeach
              </div>
            </div>
          ...
          <div class="driver-row">
            ...
            <div>
              ...
              <p class="driver-desc">{{ $driverLevels['contribution']['level_info']['desc'] ?? '' }}</p>
              <div class="tags"><span class="label">Kekuatan Anda</span>
                @foreach(array_slice($driverLevels['contribution']['level_info']['characteristics'] ?? [], 0, 3) as $tag)
                  <span>{{ $tag }}</span>
                @endforeach
              </div>
            </div>
